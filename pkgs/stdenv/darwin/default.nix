  ...
  stageFun = prevStage:
    ...
    let
      cc = if prevStage.llvmPackages.clang-unwrapped == null
        ...
        # The clang resource-root include path is intentionally unquoted, so the bootstrap tools
        # version can be `*` and treated as a glob.
        extraBuildCommands =
          let
...
[
  ...
  (prevStage:
    ...
    overrides = self: super: {
      inherit (prevStage) ccWrapperStdenv
        autoconf automake bash binutils binutils-unwrapped bison brotli cmake coreutils cpio
        curl cyrus_sasl db ed expat flex gettext gmp gnugrep groff icu libedit libffi
        libiconv libidn2 libkrb5 libssh2 libtool libunistring libxml2 m4 ncurses nghttp2
        ninja openldap openssh openssl patchutils pbzx perl pkg-config scons sed serf
        sharutils sqlite subversion unzip which xz zlib zstd;
  ...
  (prevStage:
    ...
    overrides = self: super: {
      inherit (prevStage) ccWrapperStdenv
        autoconf automake bash binutils binutils-unwrapped bison brotli cmake cpio curl icu
      ...
      darwin = super.darwin.overrideScope (_: _: {
        inherit (prevStage.darwin)
          CF ICU Libsystem configd darwin-stubs dyld launchd libclosure libdispatch libobjc
          objc4 locale postLinkSignHook print-reexports rewrite-tbd signingUtils sigtool;
      });
      ...
      llvmPackages = super.llvmPackages // (
        let
          ...
          libcxxBootstrapStdenv = self.overrideCC self.stdenv (self.llvmPackages.clangNoCompilerRtWithLibc.override {
  ...
  (prevStage:
    ...
    assert lib.all isBuiltByBootstrapFilesCompiler (with prevStage; [
      ...
      expat flex gettext gmp gnugrep groff icu libedit libtool m4 ninja openldap openssh
      patchutils pbzx perl pkg-config.pkg-config python3 scons serf sqlite subversion
      sysctl.provider unzip which xz
    ]);

    assert lib.all isBuiltByBootstrapFilesCompiler (with prevStage; [
      brotli curl libffi libiconv libiconv libidn2 libkrb5 libssh2 libunistring libxml2
      ncurses nghttp2 openssl zlib zlib zstd
    ]);
    ...
    overrides = self: super: {
      inherit (prevStage) ccWrapperStdenv
        autoconf automake bash bison brotli cmake coreutils cpio curl cyrus_sasl db ed expat
        flex gettext gmp gnugrep groff icu libedit libffi libiconv libidn2 libkrb5 libssh2
        libtool libunistring libxml2 m4 ncurses nghttp2 ninja openldap openssh openssl
        patchutils pbzx perl pkg-config python3 scons serf sqlite subversion sysctl unzip
        which xz zlib zstd;

      darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
        inherit (prevStage.darwin)
          CF ICU darwin-stubs launchd libobjc objc4 locale print-reexports rewrite-tbd sigtool
          signingUtils;
  ...
  # - x86_64-darwin: brotli curl libidn2 icu libkrb5 libssh2 libunistring nghttp2 openssl zstd
  #
  # Note: CF is built in this stage instead of along with Libsystem in the previous stage because
  # it depends on curl, which would cause an infinite recursion.
  (prevStage:
    # previous stage2 stdenv:
    assert lib.all isBuiltByBootstrapFilesCompiler (with prevStage; [
      autoconf automake bash bison brotli cmake coreutils cpio curl cyrus_sasl db ed expat
      flex gettext gmp gnugrep groff icu libedit libffi libiconv libiconv libidn2 libkrb5
      libssh2 libtool libunistring libxml2 m4 ncurses nghttp2 ninja openldap openssh openssl
      patchutils pbzx perl pkg-config.pkg-config python3 scons serf sqlite subversion
      sysctl.provider unzip which xz zlib zlib zstd
    ]);
    ...
    overrides = self: super: {
      ...
      darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
        inherit (prevStage.darwin)
          Libsystem binutils binutils-unwrapped cctools cctools-llvm cctools-port configd
          darwin-stubs dyld launchd libclosure libdispatch libtapi locale postLinkSignHook
          print-reexports rewrite-tbd signingUtils sigtool;
      });
  ...
  (prevStage:
    ...
    assert lib.all isBuiltByBootstrapFilesCompiler (with prevStage; [
      ...
      pkg-config.pkg-config python3 scons serf sqlite subversion sysctl.provider unzip which
      xz
    ]);
    ...
    assert (! useAppleSDKLibs) -> lib.all isBuiltByNixpkgsCompiler (with prevStage.darwin; [ CF ICU Libsystem ]);
    assert    useAppleSDKLibs  -> lib.all            isFromNixpkgs (with prevStage.darwin; [ CF Libsystem libobjc ]);
    assert lib.all isFromNixpkgs (with prevStage.darwin; [ dyld launchd libclosure libdispatch xnu ]);
    ...
    overrides = self: super: {
      inherit (prevStage) ccWrapperStdenv
        autoconf automake bison cmake cpio cyrus_sasl db expat flex groff libedit
        libtool m4 ninja openldap openssh patchutils pbzx perl pkg-config python3 scons serf
        sqlite subversion sysctl unzip which
  ...
  (prevStage:
    ...
    assert lib.all isBuiltByNixpkgsCompiler (with prevStage; [
      ...
      libunistring libxml2 ncurses nghttp2 openssl patch pcre xz zlib zstd
    ]);
    ...
    assert lib.all isBuiltByBootstrapFilesCompiler (with prevStage; [
      ...
      ninja openldap openssh patchutils pbzx perl pkg-config.pkg-config python3 scons serf
      sqlite subversion sysctl.provider unzip which
    ]);
    ...
    let
      ...
      persistent = self: super: {
        inherit (prevStage)
          ...
          gmp gnugrep gnumake gnused gnutar gzip icu libffi libidn2 libiconv libssh2
          libunistring libxml2 ncurses nghttp2 openssl patch pcre xz zlib zstd;

        darwin = super.darwin.overrideScope (_: _: {
          inherit (prevStage.darwin) CF ICU Libsystem dyld locale libobjc libtapi;
