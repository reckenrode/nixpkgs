...
, stdenvNoCF
, appleDerivation'
, enableStatic ? stdenvNoCF.hostPlatform.isStatic
...
appleDerivation' stdenvNoCF {
  ...
  preConfigure = lib.optionalString stdenvNoCF.hostPlatform.isiOS ''
  ...
  postInstall = lib.optionalString enableShared ''
    ...
    ${stdenvNoCF.cc.bintools.targetPrefix}install_name_tool -id $out/lib/libiconv-nocharset.dylib $out/lib/libiconv-nocharset.dylib

    # re-export one useless symbol; ld will reject a dylib that only reexports other dylibs
    echo 'void dont_use_this(){}' | ${stdenvNoCF.cc.bintools.targetPrefix}clang -dynamiclib -x c - -current_version 2.4.0 \
